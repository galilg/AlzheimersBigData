...
class Top_k():
    ...
    def __init__(self):
        ...
        self.rosmap_file = None #self.sc.parallelize([])
        self.gene_file = None #self.sc.parallelize([])

        # Top k and cluster values
        self.top_k = None #self.sc.parallelize([])
        self.allInfo = None
        self.allInfo_keyed_by_cluster = None
        self.cluster_vals = None #self.sc.parallelize([])

        # For machine learning
        self.nci_clusters = None #self.sc.parallelize([])
        self.ad_clusters = None #self.sc.parallelize([])

        # Machine learning patient top-k cluster scores with other stats
        self.nci_reduced = None #self.sc.parallelize([])
        self.ad_reduced = None #self.sc.parallelize([])

        self.ros_2_cluster = None #self.sc.parallelize([])
    ...
    def get_all_ad_and_nci_vals(self):
        return self.ad_reduced, self.nci_reduced
    ...
    def print_top_k(self):
        for cluster in range(0, len(self.top_k)):
            print("Rank #{}" .format(cluster + 1))
            print("Cluster: ", self.top_k[cluster][0])
            print("T-test score: ", self.top_k[cluster][1])
            print("AD Values (Mean, STD): ", self.top_k[cluster][2])
            print("NCI Values (Mean, STD): {} \n".format(self.top_k[cluster][3]))
    ...
    def top_k_vals_for_patients(self):
        ...
        #for item in range(0, len(self.top_k)):
        #    self.top_k[item] = (self.top_k[item], item)
